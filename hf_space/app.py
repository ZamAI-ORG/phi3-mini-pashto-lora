...
def generate_response(prompt: str, max_tokens: int = MAX_NEW_TOKENS, 
            ...
            with gr.Column(scale=2):
                ...
                with gr.Accordion("Advanced Settings", open=False):
                    max_tokens = gr.Slider(
                        minimum=50, maximum=1024, value=MAX_NEW_TOKENS,
                        label="Max Tokens", step=50
                    )
                    temperature = gr.Slider(
                        minimum=0.1, maximum=2.0, value=TEMPERATURE,
                        label="Temperature", step=0.1
                    )
                    top_p = gr.Slider(
                        minimum=0.1, maximum=1.0, value=TOP_P,
                        label="Top-p", step=0.05
                    )
            
            with gr.Column(scale=3):
                output = gr.Textbox(
                    label="Generated Response / تولید شوی ځواب",
                    lines=15,
                    max_lines=20,
                    interactive=False
                )
